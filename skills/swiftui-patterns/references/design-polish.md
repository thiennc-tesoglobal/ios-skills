...
Use the `core-haptics` skill when the interaction needs a custom waveform,
continuous feedback, live parameter modulation, an AHAP asset, or synchronized
audio and haptics. Keep standard success, warning, impact, and selection feedback
in this reference with `sensoryFeedback` or UIKit feedback generators.

Core Haptics owns capability fallback, `CHHapticEngine` lifecycle and reset
recovery, retained players, power behavior, and physical-device verification.
