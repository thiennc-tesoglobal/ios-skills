...
Test offer-code redemption in StoreKit configuration files and sandbox. In an
Xcode StoreKit Test target, use the configured offer-code reference name with
the test session API; this is not production purchase code:
...
```swift
// In a StoreKit Test target, `testSession` is an SKTestSession.
import StoreKitTest

// Simulate Ask to Buy
try await product.purchase(options: [.simulatesAskToBuyInSandbox(true)])

// Test-only: set purchase date and renewal behavior through StoreKit Test.
try await testSession.buyProduct(
    identifier: product.id,
    options: [.purchaseDate(Date(), renewalBehavior: .renewUntilNow)]
)

// Test-only: apply an offer code by reference name through StoreKit Test.
try await testSession.buyProduct(
    identifier: product.id,
    options: [.codeOffer(referenceName: "SUMMER2024")]
)
...
Use `product.purchase(options:)` for app purchases and `SKTestSession` options
only in a StoreKit Test target.
