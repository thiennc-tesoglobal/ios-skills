...
class AppDelegate: NSObject, UIApplicationDelegate {
    ...
    func application(
        ...
        // Supported current Simulator hosts can register with APNs Sandbox.
        // A failure is host/runtime/network-specific; .apns and simctl remain
        // available for provider-free simulation.
        print("Simulator APNs registration failed: \(error.localizedDescription)")
        #else
...
### Simulator Coverage and Limitations

- On supported current Xcode/OS hosts, iOS Simulator can register with APNs Sandbox and receive a simulator-specific, variable-length token.
- Provider delivery to that Simulator can exercise more of the remote-notification path than a local payload simulation.
- Drag-and-drop `.apns` files and `xcrun simctl push` simulate delivery without exercising the provider/APNs path.
- Local notifications work normally on the simulator.
- Registration failures are not universal Simulator behavior; inspect the host, runtime, signing, network, and CI/VM constraints.
- Verify production APNs environment, entitlements, and hardware-specific behavior on a physical device.
