...
class NotificationService: UNNotificationServiceExtension {
    ...
    override func didReceive(
        ...
        processingTask = Task { [weak self] in
            ...
            await self.processNotification(content: content)
            self.finish(content)
        }
    }
    ...
    override func serviceExtensionTimeWillExpire() {
        finish(bestAttemptContent ?? fallbackContent)
    }

    private func finish(_ content: UNNotificationContent?) {
        finishLock.lock()
        guard !didComplete, let content else {
            finishLock.unlock()
            return
        }
        didComplete = true
...
class NotificationService: UNNotificationServiceExtension {
    ...
    private func finish(with content: UNNotificationContent?) {
        finishLock.lock()
        guard !didComplete, let content, let handler = contentHandler else {
            finishLock.unlock()
            return
        }
        didComplete = true
